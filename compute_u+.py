...
#   Parâmetros 
    k = 2.50          # Número de onda
    a = 1.0          # Raio do disco
    n_points = 100  # Número de pontos na fronteira
...
# compute u+
    ...
    for i, xi in enumerate(xi_points):
        ...
        u_scat_field[i] = calculate_u_plus_integral(xi, k, n_quad, mesh, p_values)
...
# calculo do erro relativo em xi_vld
    u_scat_numerical = calculate_u_plus_integral(xi_vld, k, n_quad, mesh, p_values)
...
# vérifier la condition de dirichlet
    theta_on_boundary = Theta[:, 0]
    u_inc_on_boundary = u_inc_grid[:, 0]
    u_scat_on_boundary = u_scat_grid[:, 0]
    u_total_on_boundary = u_total_grid[:, 0] # Ou u_inc_on_boundary + u_scat_on_boundary

    # --- 2. Calcular as magnitudes ---
    mag_inc = np.abs(u_inc_on_boundary)
    mag_scat = np.abs(u_scat_on_boundary)
    mag_total = np.abs(u_total_on_boundary)

    # --- 3. Imprimir valores médios para diagnóstico ---
    print(f"Magnitude média de u_inc na borda: {np.mean(mag_inc):.4f}")
    print(f"Magnitude média de u_scat na borda: {np.mean(mag_scat):.4f}")
    print(f"Magnitude média de u_total na borda: {np.mean(mag_total):.4e}")
